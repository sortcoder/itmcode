...
<form action="" method="post" id="createUser">
    ...
    @method('PUT')
    <div class="card">
        <div class="card-header"><h5>Update Launch Paid Package </h5></div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-4 mt-2">
                    ...
                    <input type="number" step="any" class="form-control" name="quantity" value="{{ $row->quantity }}" id="quantity" placeholder="Enter Quantity">
                </div>
                <div class="col-4 mt-2">
                    <label for="price">One Image Price</label>
                    <input type="number" min="0" step="any" class="form-control" name="one_image_price" id="one_image_price" placeholder="Enter One Image Price" value="{{ $row->one_image_price }}" >
                </div>
                <div class="col-4 mt-2">
                    <label for="price">Package Price</label>
                    <input type="number" min="0" step="any" class="form-control" value="{{ $row->price }}" name="price" id="price" placeholder="Enter Package Price">
                </div>
	...
	$('#createUser').validate({
		...
		submitHandler:function(form,event){
			...
			$.ajax({
					...
					url: '{{  route("package.update",[encodeId($row->id)]) }}',
					type: 'post',
					data: data,
					cache:false,
					contentType:false,
