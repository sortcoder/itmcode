				...
				<div class="row">
					...
					<div class="col-12">
                        <div class="form-group">
                            ...
                            $rolePermissions = !empty($user->permission_ids)?explode(',',$user->permission_ids):[];
                            ?>

                            @foreach($permission as $value)
                                <label>{{ Form::checkbox('permission_ids[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                                {{ $value->name }}</label>
                            <br/>
                            @endforeach
                        </div>
	...
	$('#createUser').validate({
		...
		submitHandler:function(form,event){
			...
			$.ajax({
					...
					url: '{{ route("module.update",[encodeId($user->id)]) }}',
					type: 'post',
					data: data,
					cache:false,
					contentType:false,
