...
<?php
  ...
  $data = [];
  $msg = '';
        $response = \App\Models\StockExchange::select(DB::raw('DATE(created_at) as date'), DB::raw('MAX(price) as high'), DB::raw('MIN(price) as low'))->where('status',"successful")->where('lauch_id',$row->id)->groupBy('date')->get()->toArray();
